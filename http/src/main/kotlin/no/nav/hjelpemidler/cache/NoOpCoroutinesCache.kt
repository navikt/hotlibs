...
class NoOpCoroutinesCache<K : Any, V> : CoroutinesCache<K, V> {
    ...
    override suspend fun get(key: K, loader: suspend CoroutineScope.(K) -> V): V = coroutineScope { loader(key) }
    ...
    override suspend fun compute(key: K, loader: suspend CoroutineScope.(K, V?) -> V): V? =
        coroutineScope { loader(key, null) }
