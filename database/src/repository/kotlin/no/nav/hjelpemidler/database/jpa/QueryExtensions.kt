...
fun <T, R> TypedQuery<T>.map(transform: (T) -> R): List<R> = resultList.map(transform)

fun <T, R, C : MutableCollection<in R>> TypedQuery<T>.mapTo(destination: C, transform: (T) -> R): C =
    resultList.mapTo(destination, transform)

fun <T, R : Any> TypedQuery<T>.mapNotNull(transform: (T) -> R?): List<R> = resultList.mapNotNull(transform)
...
fun <T> TypedQuery<T>.setParameters(vararg pairs: Pair<String, Any?>): TypedQuery<T> = setParameters(pairs.toMap())
