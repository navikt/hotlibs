...
infix fun <T : LogiskOperand<T>> Evaluering<T, *>.harResultat(expected: T) =
    resultat shouldBe expected

infix fun <T : LogiskOperand<T>> Evaluering<T, *>.harBegrunnelse(expected: String) =
    begrunnelse shouldBe expected

infix fun <T : LogiskOperand<T>> Evaluering<T, *>.harOperator(expected: Operator) =
